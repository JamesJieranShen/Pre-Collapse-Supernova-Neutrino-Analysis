...
void energyIntegral(const TH2D &hist, double background, bool include_bg)
{
    ...
    for (int i = 1; i <= hist.GetNbinsX(); i++)
    {
        ...
        {
            output.Fill(hist.GetXaxis() -> GetBinCenter(i), hist.GetBinContent(i, j)); // * ( hist.GetYaxis()->GetBinUpEdge(i) - hist.GetYaxis()->GetBinLowEdge(i) ) );
        }
        signalcounter[i] = output.GetBinContent(i);

        double bg = background * (pow(10, -hist.GetXaxis() -> GetBinLowEdge(i)) - pow(10, -hist.GetXaxis() -> GetBinLowEdge(i + 1)));

        if (include_bg)
            output.Fill(hist.GetXaxis() -> GetBinCenter(i), bg);

        BG.Fill(hist.GetXaxis() -> GetBinCenter(i), bg);
