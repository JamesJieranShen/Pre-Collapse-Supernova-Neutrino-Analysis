...
void drawbg(std::string filepath, std::string detector, std::string ordering, double sigmaUp, bool include_bg)
{
    ...
    for (int i = 0; i < thl; i++)
    {
        ...
        for (int j = 1; j < bgl; j++)
        {
            ...
            if (include_bg)
                savepath = savepath + std::string("_withbg");
            ...
            if (include_bg)
                histname = histname + std::string("_withbg");
            ...
            //double right_max = log10(sigma->GetMaximum());
            //cout << right_max << endl;
            double scale = gPad->GetUymax() / sigmaUp;  // scale between two verticle axis
